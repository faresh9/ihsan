...
import { CALENDAR_EVENT_STYLE } from './util'; // Assuming this file contains styles for different themes
  ...
  return (
    <div className="w-full bg-base-100 p-4 rounded-lg">
      <div className="flex items-center justify-between">
        ...
        <div>
          <button className="btn btn-sm btn-ghost btn-outline normal-case" onClick={() => addNewEvent(moment().format())}>
            Add New Event
          </button>
        </div>
      </div>
          ...
          <div key={idx} className={`${colStartClasses[moment(day).day().toString()]} border border-solid w-full h-28`}>
            <p
              ...
              className={`inline-block flex items-center justify-center h-8 w-8 rounded-full mx-1 mt-1 text-sm cursor-pointer hover:bg-base-300 ${
                isToday(day) ? "bg-primary text-primary-content" : isDifferentMonth(day) ? "text-base-content/50" : "text-base-content"
              }`}
            ...
            {getEventsForCurrentDate(day).map((e, k) => (
              <p key={k} onClick={() => openAllEventsDetail(day, e.theme)} className={`text-xs px-2 mt-1 truncate ${THEME_BG[e.theme] || ""}`}>
                {e.title}
              </p>
            ))}
          </div>
